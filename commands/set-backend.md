---
description: Set the prover backend used by /autoform:orchestrate (max | aristotle | codex) — persisted, so you set it once. Backend is also the billing path (max = Max subscription, never the API). Shared with the dashboard's backend dropdown.
...
changes. This command persists which backend `/autoform:orchestrate` uses, so you pick it **once**
...
- **`<backend>`** (`max` | `aristotle` | `codex`) → `backend_config.py set <backend>`, then **echo the
  ...
  what `/autoform:orchestrate` will run on. An unknown backend errors with the known list; `codex`
  persists but warns its adapter isn't built yet.

The setting lives at `~/.autoform/config.json` (override with `$AUTOFORM_CONFIG`).
`/autoform:orchestrate` reads it on each run; an explicit `--backend` on dispatch still overrides it.
