...
Defaults:
    ...
    --template: <repo>/visualization/templates/dep_graph.html

This emits files only; the toolchain runs separately.

graph.json schema (see internal/references/plan-json-schema.md), the fields we read:
    top level:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--template", type=Path, default=None,
                        help="custom dep_graph.html (default: <repo>/visualization/templates/dep_graph.html)")
    ...
    # Keep the renderer and its template together under visualization/.
    visualization_root = Path(__file__).resolve().parent
    template_path = (args.template or visualization_root / "templates" / "dep_graph.html").resolve()
    ...
    _core_js = visualization_root / "assets" / "dep_graph_core.js"
    if _core_js.is_file():
