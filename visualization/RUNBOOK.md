...
```bash
"<AUTOFORM_PLUGIN_ROOT>/visualization/check_toolchain.sh"
```
...
`<AUTOFORM_PLUGIN_ROOT>/visualization/SETUP.md` for the manual path. Do not attempt later steps
...
uv run --directory "<AUTOFORM_PLUGIN_ROOT>" python \
  "<AUTOFORM_PLUGIN_ROOT>/visualization/export_blueprint.py" <graph.json> \
  [--content <dir>] [--out <dir>]
```
...
```bash
uv run --directory "<AUTOFORM_PLUGIN_ROOT>" python \
  "<AUTOFORM_PLUGIN_ROOT>/visualization/service_control.py" start blueprint \
