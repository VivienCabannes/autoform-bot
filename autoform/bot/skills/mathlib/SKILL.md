...
- **`unproved`** — "The book doesn't prove this." Use the `unproved` macro (`unproved theoremName (args) : Conclusion`) when the book explicitly omits the proof, references another source, or states without proof. Compiles to `@[unproved] axiom` so the infrastructure tracks it as a justified gap. **Best option** for statements without proofs in the source material.
- **`axiom`** — Use for infrastructure the worker fails to construct (definitions, structures, instances, helper lemmas). Compiles cleanly without poisoning the kernel. Prefer over `sorry` when you need to move on.
- **`sorry`** — **Avoid.** Introduces `sorryAx` which breaks soundness for everything downstream. Use only as a temporary placeholder during active proof development, never as a final state.
