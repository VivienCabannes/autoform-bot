...
against the :class:`~autoform.prover.base.ProverAdapter` interface and the shared
:class:`~autoform.prover.steerer.Steerer` **only**. It contains **zero**
backend-specific code — per-backend behaviour differences are keyed off the
adapter's declared :class:`~autoform.prover.base.SteeringCapability`, never off
...
3. Every event also feeds the **structured trigger engine**
   (:mod:`autoform.prover.triggers`) — deterministic signals (repeated build
    ...
    *,
    ...
    Args:
        ...
        verifier: The honesty gate run on a *claimed* ``proved`` — it independently
            ...
            (and tests inject a fake). Defaults to :func:`autoform.prover.verify.verify_proof`.
