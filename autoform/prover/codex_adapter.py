...
its event stream onto the shared :class:`~autoform.prover.base.Event` vocabulary,
steer turn-granularly by resuming the session, and parse the final report into a
:class:`~autoform.prover.base.ProofResult` — held to the SAME no-cheating /
...
class CodexAdapter(ProverAdapter):
    ...
    Args mirror :class:`~autoform.prover.claude_adapter.ClaudeAdapter`. ``runner`` is
