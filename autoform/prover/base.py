...
(:mod:`autoform.prover.driver`) and the *steering judge*
(:mod:`autoform.prover.steerer`) are written **against this interface alone**, so
...
class Event:
    ...
    Args:
        ...
        path: For ``EDIT`` (and file-touching ``TOOL``) events: the file path
            ...
            steering triggers (:mod:`autoform.prover.triggers`) use it for
...
class SteeringCapability(str, Enum):
    ...
    :mod:`autoform.prover.driver`), so the loop stays backend-agnostic while doing
...
class ProverAdapter(abc.ABC):
    ...
    * :class:`autoform.prover.claude_adapter.ClaudeAdapter`
    * :class:`autoform.prover.aristotle_adapter.AristotleAdapter`
    * :class:`autoform.prover.codex_adapter.CodexAdapter`
