...
* ``events`` — parse the stream-json lines into normalized
  :class:`~autoform.prover.base.Event`\\ s. Captures the ``session_id`` from the
...
* ``result`` — the final assistant text (the Lean proof, or an honest ``FAILED``)
  parsed into a :class:`~autoform.prover.base.ProofResult`.
...
class ClaudeAdapter(ProverAdapter):
    ...
    #: via the verify-gate fold. See :class:`~autoform.prover.base.SteeringCapability`.
