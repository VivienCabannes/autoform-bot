...
A thin :class:`~autoform.prover.base.ProverAdapter` over the Aristotle
integration in :mod:`autoform.prover.aristotle`. It exposes the
...
* ``events`` — poll the running task and yield each new Aristotle event,
  normalized to a :class:`~autoform.prover.base.Event`; ends when the task reaches
  a terminal status.
...
* ``result`` — on terminal status, land the returned Lean files into the project,
  ...
  :class:`~autoform.prover.base.ProofResult`.
...
class AristotleAdapter(ProverAdapter):
    ...
    Args:
        ...
        system_prompt: The no-cheating delegate contract (defaults to
            :data:`autoform.prover.aristotle.DEFAULT_DELEGATE_SYSTEM`).
