...
2. Sets each tier-2 node's `parent`, `statement_depends_on`, and `proof_depends_on` (referencing prerequisite nodes by id), and proposes a `mathlib_status` guess for each. `merge_node.py` maintains `depends_on` as their scheduler union. A node that fits no existing cluster gets `parent: null` for now, or names a proposed new cluster; the re-projection step settles its home.
