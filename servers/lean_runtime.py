...
from servers.repl.core import DEFAULT_MAX_CONTEXTS_PER_PROCESS, format_repl_response
...
class LeanRuntimeConfig:
    ...
    def from_environment(cls) -> "LeanRuntimeConfig":
        ...
        if repl_max_contexts < 3:
            raise ValueError(
                "AUTOFORM_REPL_MAX_CONTEXTS_PER_PROCESS must be at least 3"
            )
        ...
        if (
            max_repl_request_seconds + RUNTIME_SAFETY_SECONDS > response_timeout
        ):
            raise ValueError(
...
class ProjectResourceCache(Generic[T]):
    ...
    def _acquire(
        ...
        *,
            ...
            with self._condition:
                ...
                if not wait and entry is not None:
                    try:
                        ...
                        if required_fingerprint is not None:
                            raise ProjectResourceBusyError(
                                "shared Lean project changed after validation: "
                                f"{root}"
                            ) from error
                        raise
                    if (
            ...
            if deadline is None:
                ...
                try:
                    ...
                    if future.done():
                        ...
                        def publish_late_result(completed: Future[T]) -> None:
                            try:
                        ...
                        future.add_done_callback(publish_late_result)
                        late_creation = True
        except BaseException:
        ...
        startup_expired = deadline is not None and self._clock() >= deadline
        disposition = self._settle_created_resource(
            root,
            created,
            fingerprint=fingerprint,
            required_fingerprint=required_fingerprint,
            active=not startup_expired,
        )
    ...
    def _settle_created_resource(
        ...
        *,
        ...
        """Publish a valid startup, even when its original caller timed out."""
        try:
            current_fingerprint = lean_project_fingerprint(root)
        except OSError:
        ...
        with self._condition:
            if self._closed:
                ...
                disposition = "changed"
            elif root not in self._creating or root in self._entries:
                disposition = "closed"
            else:
        ...
        if disposition != "published":
            self._discard_created_resource(root, resource)
        return disposition
...
class LeanRuntimeServices:
    ...
    def _require_lsp_fingerprint(
        ...
        try:
            current = lean_project_fingerprint(root)
        ...
        if current != expected_project or current_file != expected_file:
            raise ProjectResourceBusyError(
                f"shared Lean project or target changed during LSP request: {root}"
            )
...
class _ThreadingUnixServer(socketserver.ThreadingUnixStreamServer):
    # Every request is deadline-bounded. Joining handlers on close keeps the
    # lifetime lock held until admitted Lean calls finish.
