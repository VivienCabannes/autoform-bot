...
|---|---|---|
| `python -m autoform.statement_extraction` | Removed | Put source material in the project and run `roadmap`, which owns planning. Review the resulting `graph.json`; no automatic `targets.yaml` conversion is provided. |
| `python -m autoform.bot.main` and YAML run configuration | Removed | Run the host-native `setup`, `roadmap`, and `orchestrate` workflows. Orchestrate owns backend selection; durable state lives beside the Lean project. |
