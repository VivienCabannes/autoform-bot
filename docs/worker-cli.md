...
autoform sync   [--json]            # fold merged scoreboards locally (no push)
...
machine converges on its next round. The graph advanced by two nodes with zero
human coordination — the humans watched it on the roadmap site, and either could
have held the gate from their review dashboard at any point.
