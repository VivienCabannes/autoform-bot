...
def test_discovery_rejects_symlinked_docker_executable(
    provider_environment: _Environment,
) -> None:
    link = provider_environment.docker.parent / "docker-link"
    link.symlink_to(provider_environment.docker)

    with pytest.raises(GateProviderError, match="canonical non-symbolic"):
        _discover(provider_environment, _Runner(provider_environment), docker_executable=os.fspath(link))
...
def test_discovery_resolves_a_bare_docker_name_from_path(
    provider_environment: _Environment,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("PATH", os.fspath(provider_environment.docker.parent))
    config = _discover(provider_environment, _Runner(provider_environment), docker_executable="docker")
    assert config.runtime_path == os.fspath(provider_environment.docker)
...
def test_revalidation_rejects_semantic_drift_after_socket_replacement(
    ...
    with pytest.raises(GateProviderError, match="semantic fingerprint"):
        revalidate_docker_gate_provider(
            config,
            command_timeout_seconds=7,
            command_output_bytes=131072,
