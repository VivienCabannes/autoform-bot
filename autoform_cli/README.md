...
| `mathlib_declaration: Ns.name` | Verified matching declaration(s) in Mathlib. |
| `mathlib_file: Mathlib/Path.lean` | Verified primary Mathlib source file. |
...
```bash
autoform-blueprint check blueprint --lean-root .
```
...
```bash
autoform-blueprint render blueprint --output site-src --lean-root . --require-declarations
```
...
`autoform-blueprint render` publishes three views of the same committed Markdown: the
