...
def create_project(
    target: str | Path | None, *, package: str | None, release_id: str | None
) -> ProjectCreateResult:
    ...
    try:
        ...
        workspace_identity = _descriptor_identity(workspace_descriptor)
        ...
        try:
            written, workflows_pinned = _build_staged_project(stage_path, package_name, release)
            _require_stage_identity(parent_descriptor, workspace_name, workspace_descriptor)
...
def _build_staged_project(
    stage: Path, package: str, release: SupportedRelease
    ...
    scaffold = scaffold_project(
        stage,
        title=package,
        autoform_source=DEFAULT_AUTOFORM_SOURCE,
        autoform_ref="",
