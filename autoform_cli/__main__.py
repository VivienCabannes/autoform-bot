...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    project_repair.add_argument("target", help="existing project root")
    ...
    project_repair.add_argument(
        "--autoform-source",
        help="verified Autoform HTTPS Git source for missing workflows",
    )
    project_repair.add_argument(
        "--autoform-ref",
        help="verified full Autoform commit SHA for missing workflows",
    )
...
def _project(args: argparse.Namespace) -> int:
    try:
        ...
        if getattr(args, "json", False):
            ...
            if error.written:
                print("  files already or possibly published:", file=sys.stderr)
