...
When creating a new blueprint, read the concise [Cabannes thesis example](references/thesis-blueprint.md) and adapt its repo-shaped [blueprint asset](assets/cabannes-thesis-project/blueprint/README.md); do not copy its mathematics. Keep the convention transparent and editable:
...
Preserve and merge existing Markdown, workflows, and ignore rules; never overwrite them wholesale. Treat generated HTML and site output as disposable. After changes, run the bundled checker and exporter plus the adapted MkDocs build, then report the vault path, generated views, Pages workflow, and any remaining one-time repository setting.
